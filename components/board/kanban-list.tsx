...
import { Plus, MoreHorizontal, X, ChevronsRight, ChevronsLeft } from 'lucide-react'
...
export function KanbanList({ list, cards, onCardClick }: KanbanListProps) {
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`flex-shrink-0 flex flex-col max-h-[calc(100vh-180px)] rounded-2xl border transition-all duration-300 ${
    ...
    >
      {isCollapsed ? (
        // COLLAPSED VIEW
        <div 
          className="flex-1 flex flex-col items-center py-4 cursor-pointer group hover:bg-white/5 transition-colors rounded-2xl"
          onClick={() => setIsCollapsed(false)} // Expand on click
        >
          {/* Drag handle */}
          <div className="absolute inset-0 cursor-grab active:cursor-grabbing" {...attributes} {...listeners} />
          
          <Button variant="ghost" size="icon" className="h-8 w-8 hover:bg-white/10 text-white rounded-lg mb-3 relative z-10 pointer-events-none">
            <ChevronsLeft className="h-5 w-5 text-white/70 group-hover:text-white transition-colors" />
          </Button>

          <span className="text-xs font-semibold text-white/70 px-2 py-1 rounded-md bg-black/20 shadow-inner relative z-10 mb-4 pointer-events-none">
            {cards.length}
          </span>
          
          <div className="flex-1 relative z-10 overflow-hidden mt-2 pointer-events-none">
             <span 
               className="font-bold text-base text-white/70 group-hover:text-white transition-colors whitespace-nowrap block truncate max-w-[60vh]"
        ...
        <>
          {/* List Header */}
          <div className="p-4 flex items-center justify-between cursor-grab active:cursor-grabbing shrink-0 group" {...attributes} {...listeners}>
            <div className="flex items-center gap-2">
               ...
               {isEditingTitle ? (
                 ...
                 <button onClick={() => setIsEditingTitle(true)} className="font-bold text-base text-white hover:text-primary px-1 -ml-1 rounded transition-colors drop-shadow-sm truncate max-w-[150px]">
                   {list.title}
                 </button>
               )}
            </div>
            ...
            <div className="flex items-center gap-1">
              ...
              <Button 
                variant="ghost" 
                size="icon" 
                className="h-8 w-8 text-white rounded-lg opacity-0 group-hover:opacity-100 hover:bg-white/10 transition-opacity z-10"
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="ghost" size="icon" className="h-8 w-8 hover:bg-white/10 text-white rounded-lg z-10"><MoreHorizontal className="h-5 w-5" /></Button>
                </DropdownMenuTrigger>
